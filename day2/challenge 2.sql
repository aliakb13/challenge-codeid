...
foreign key (cart_id) references oe.carts (cart_id)
 ...
 add constraint bank_code_fk foreign key (bank_code) references oe.banks(bank_code),
...
UPDATE oe.users SET user_email = lower(user_name || '' || '@gmail.com');
...
-- SELECT od.ship_address, locx.street_address FROM oe.orders od JOIN oe.location_x locx ON od.ship_address = locx.street_address; 
